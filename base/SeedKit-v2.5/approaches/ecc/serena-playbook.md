# Serena Playbook — semantic code navigation for ECC projects

> The harness can register the Serena MCP and never use it. This playbook exists to make sure
> Serena is the **default** for code exploration and symbol-level edits, not an afterthought.
> Read by every PRP command and by `/plan` so the symbol-first habit propagates into every workflow.
...
loaded as an always-on reference and being explicitly cited from `/prp-plan`, `/prp-implement`,
and `/plan`.
...
To prevent that, ECC operates on a **three-layer memory model** with an explicit
maintenance ritual wired into the PRP loop. The project's own `CLAUDE.md` codifies the
...
| `suggested_commands`      | One-time setup, dev/build/test/migration commands, phase-close grep gates, PRP + Serena cheat-sheet                  | A new runtime script lands or a grep gate changes                              |
...
Codified in the project `CLAUDE.md` as **a blocker before `/prp-commit`**. Before the
...
**Why it's a blocker for `/prp-commit`**: the memory writes land in the same commit as
...
**Why it's attached to the PRP loop specifically**: `/prp-implement` is the only command
...
- Ephemeral task progress — that belongs in the phase report or ECC session memory.
...
don't try to backfill retroactively outside the PRP loop.
...
- Anything in the seed/, plans/, .claude/PRPs/ folders — those are docs, use `Read`.
...
## How PRP commands and `/plan` use this playbook

The PRP commands (`/prp-plan`, `/prp-implement`) and `/plan` are patched at bootstrap to include
this preamble at the top of their prompt body:

```markdown
> **Serena-first.** Before any code exploration, read `serena-playbook.md` once per session.
> For every code-reading or code-editing decision in this command, consult its decision matrix.
> Default: `get_symbols_overview` over `Read` for files > ~150 lines.
> For any rename or signature change planned by this command, run `find_referencing_symbols`
> first and include the call sites in the plan.
```

This makes the symbol-first habit propagate into every workflow without relying on Claude
"remembering" a generic rule from `CLAUDE.md`.
...
- [ ] The project's `CLAUDE.md` contains **both** the §Verification ritual memory-hygiene
      step (a blocker before `/prp-commit`) **and** the §Mid-phase decision memories
...
the Serena-first preamble is present at the top of `/prp-plan`, `/prp-implement`, and
`/plan`, and make sure the bootstrap emitted the memory-hygiene ritual into the project
`CLAUDE.md` (it's part of the template since the first three-layer-model update — any
pre-update project can cherry-pick it from `SeedKit-v2/approaches/ecc/CLAUDE.md`).
